#!/usr/bin/env bash
# AIR Performance Prompt 13/16 R4 machine evidence collector.
# Compares qualified reuse8 against the memory-heavier dense FP32 cuBLAS research tactic.
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
STAMP="$(date +%Y%m%d-%H%M%S)"
OUT="$HOME/Downloads/AIR-Performance13-R4-$STAMP"
ARCHIVE="$HOME/Downloads/AIR-Performance13-R4-$STAMP.zip"
BUILD="${AIR_PERF13_R4_BUILD_DIR:-$ROOT/build-performance13-r4}"
PROFILE_BUILD="${AIR_PERF13_R4_PROFILE_BUILD_DIR:-$ROOT/build-performance13-r4-profile}"
...
{
 echo harness=AIR-Performance13-R4
 echo date="$(date -Is)"
 echo air_version_expected=0.9.8
 echo benchmark_schema_expected=air.benchmark.v9
...
cleanup; trap - EXIT
{
 ...
 echo note='R4 fixes dense block-linear tactic scope; output projection remains quantized. dense-f32-cublas remains a diagnostic/Pareto candidate.'
} >"$OUT/RESULT.txt"

cp "$ROOT/docs/PROMPT13_SHARED_TILE_RESULT.md" "$OUT/source/"
cp "$ROOT/docs/PROMPT13_DENSE_F32_R3_RESULT.md" "$OUT/source/"
